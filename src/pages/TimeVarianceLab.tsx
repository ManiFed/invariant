...
import { ArrowLeft, Play, Pause, SkipBack, SkipForward, Clock, Plus, Trash2, ChevronRight } from "lucide-react";
...
function interpolateParams(keyframes: Keyframe[], t: number): { weightA: number; weightB: number; feeRate: number; amplification: number } {
...
/* ─── Curve computation ─── */
function computeCurve(wA: number, wB: number, k: number): { x: number; y: number }[] {
  const pts: { x: number; y: number }[] = [];
  for (let i = 1; i <= 50; i++) {
    const x = i * 4;
    const y = Math.pow(k / Math.pow(x, wA), 1 / wB);
    if (y > 0 && y < k * 10 && isFinite(y)) {
...
function computeTimeline(keyframes: Keyframe[], reserveX: number, reserveY: number): TimeSnapshot[] {
  ...
  for (let t = 0; t <= 100; t += 1) {
    ...
    const x = reserveX * (1 + 0.3 * Math.sin(t * 0.1)); // simulated x variation
    const yCalc = Math.pow(k / Math.pow(x, params.weightA), 1 / params.weightB);
    const price = isFinite(yCalc / x) ? yCalc / x : 1;
    snaps.push({
      t,
      x: parseFloat(x.toFixed(2)),
      y: parseFloat((isFinite(yCalc) && yCalc > 0 ? yCalc : reserveY).toFixed(2)),
      price: parseFloat(price.toFixed(4)),
      weightA: parseFloat(params.weightA.toFixed(4)),
      weightB: parseFloat(params.weightB.toFixed(4)),
      feeRate: parseFloat(params.feeRate.toFixed(4)),
      amp: parseFloat(params.amplification.toFixed(2)),
    });
  }
  return snaps;
}
...
function TimelineEditor({ keyframes, onChange, currentTime }: {
  keyframes: Keyframe[];
  ...
  const addKeyframe = () => {
    ...
    onChange([...keyframes, newKf].sort((a, b) => a.t - b.t));
  };
  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center justify-between">
        ...
        <button
          onClick={addKeyframe}
          className="inline-flex items-center gap-1 px-2 py-1 text-[10px] font-medium rounded-md bg-primary text-primary-foreground hover:opacity-90 transition-opacity"
        >
      ...
      <div className="relative h-8 rounded-lg bg-secondary border border-border overflow-hidden">
        ...
        {keyframes.map(kf => (
          <div
            key={kf.id}
            className="absolute top-1 bottom-1 w-2 rounded-full bg-chart-2 border border-background cursor-pointer hover:scale-125 transition-transform"
            style={{ left: `calc(${kf.t}% - 4px)` }}
            title={`t=${kf.t}`}
          />
        ))}
      </div>
        ...
        {keyframes.map((kf, i) => (
          <div key={kf.id} className="p-2 rounded-lg bg-background border border-border space-y-1.5">
            <div className="flex items-center justify-between">
              ...
              {keyframes.length > 1 && (
                <button onClick={() => removeKeyframe(kf.id)} className="text-muted-foreground hover:text-destructive transition-colors">
            ...
            <div className="grid grid-cols-2 gap-x-3 gap-y-1">
              <div>
                <label className="text-[8px] text-muted-foreground">Time (0–100)</label>
                <input type="number" min={0} max={100} value={kf.t}
                  onChange={e => updateKeyframe(kf.id, "t", Number(e.target.value))}
                  className="w-full px-1.5 py-0.5 text-[9px] font-mono bg-secondary rounded border border-border text-foreground" />
              </div>
              <div>
                <label className="text-[8px] text-muted-foreground">Weight A</label>
                <input type="number" step={0.05} min={0.01} max={0.99} value={kf.weightA}
                  onChange={e => updateKeyframe(kf.id, "weightA", Number(e.target.value))}
                  className="w-full px-1.5 py-0.5 text-[9px] font-mono bg-secondary rounded border border-border text-foreground" />
              </div>
              <div>
                <label className="text-[8px] text-muted-foreground">Weight B</label>
                <input type="number" step={0.05} min={0.01} max={0.99} value={kf.weightB}
                  onChange={e => updateKeyframe(kf.id, "weightB", Number(e.target.value))}
                  className="w-full px-1.5 py-0.5 text-[9px] font-mono bg-secondary rounded border border-border text-foreground" />
              </div>
              <div>
                <label className="text-[8px] text-muted-foreground">Fee Rate</label>
                <input type="number" step={0.001} min={0} max={0.1} value={kf.feeRate}
                  onChange={e => updateKeyframe(kf.id, "feeRate", Number(e.target.value))}
                  className="w-full px-1.5 py-0.5 text-[9px] font-mono bg-secondary rounded border border-border text-foreground" />
              </div>
              <div className="col-span-2">
                <label className="text-[8px] text-muted-foreground">Amplification</label>
                <input type="number" step={1} min={1} max={1000} value={kf.amplification}
                  onChange={e => updateKeyframe(kf.id, "amplification", Number(e.target.value))}
                  className="w-full px-1.5 py-0.5 text-[9px] font-mono bg-secondary rounded border border-border text-foreground" />
              </div>
            </div>
          </div>
      ...
      <div className="p-2.5 rounded-lg bg-secondary border border-border space-y-1">
        ...
        <p className="text-[8px] text-muted-foreground">
          w₁(t) and w₂(t) interpolate between keyframes. Fee rate and amplification also vary.
        </p>
      </div>
    </div>
  );
...
function PlaybackControls({ time, isPlaying, speed, onTimeChange, onTogglePlay, onSpeedChange, onReset, onStepForward }: {
  time: number;
  isPlaying: boolean;
  speed: number;
  ...
  return (
    <div className="p-3 rounded-lg bg-secondary border border-border space-y-2">
      <div className="flex items-center gap-2">
        <button onClick={onReset} className="p-1.5 rounded-md hover:bg-background text-muted-foreground hover:text-foreground transition-colors">
          <SkipBack className="w-3.5 h-3.5" />
        </button>
        <button onClick={onTogglePlay} className="p-1.5 rounded-md bg-primary text-primary-foreground hover:opacity-90 transition-opacity">
          {isPlaying ? <Pause className="w-3.5 h-3.5" /> : <Play className="w-3.5 h-3.5" />}
        </button>
        <button onClick={onStepForward} className="p-1.5 rounded-md hover:bg-background text-muted-foreground hover:text-foreground transition-colors">
          <SkipForward className="w-3.5 h-3.5" />
        </button>
        <div className="flex-1">
          <Slider min={0} max={100} step={1} value={[time]} onValueChange={([v]) => onTimeChange(v)} />
        </div>
        ...
        {[0.5, 1, 2, 5].map(s => (
          <button
            ...
            className={`px-2 py-0.5 rounded text-[9px] font-mono transition-all ${
              speed === s ? "bg-primary text-primary-foreground" : "text-muted-foreground hover:text-foreground hover:bg-background"
            }`}
          >
            {s}x
          </button>
...
const TimeVarianceLab = () => {
  ...
  const [reserveX] = useState(1000);
  const [reserveY] = useState(1000);
  const [vizTab, setVizTab] = useState("curve");
  ...
  const k = 10000;
  const curveData = useMemo(() => computeCurve(currentParams.weightA, currentParams.weightB, k), [currentParams]);
      ...
      <header className="border-b border-border px-6 py-3 flex items-center justify-between shrink-0">
        ...
        <div className="flex items-center gap-3">
          <div className="px-3 py-1 rounded-lg bg-secondary border border-border text-[10px] font-mono text-foreground">
            t={Math.round(currentTime)} | w₁={currentParams.weightA.toFixed(2)} w₂={currentParams.weightB.toFixed(2)} | fee={currentParams.feeRate.toFixed(3)}
          </div>
          <ThemeToggle />
        </div>
      </header>
      ...
      <div className="flex flex-1 min-h-0">
        ...
        <aside className="w-80 border-r border-border p-4 overflow-y-auto shrink-0 space-y-4">
          <PlaybackControls
            ...
            onStepForward={() => setCurrentTime(prev => Math.min(100, prev + 5))}
          />
        ...
        <main className="flex-1 p-4 overflow-y-auto space-y-4">
          <Tabs value={vizTab} onValueChange={setVizTab}>
            <TabsList className="h-8">
              <TabsTrigger value="curve" className="text-[10px] px-3 py-1">Invariant Curve</TabsTrigger>
              <TabsTrigger value="params" className="text-[10px] px-3 py-1">Parameter Evolution</TabsTrigger>
              <TabsTrigger value="price" className="text-[10px] px-3 py-1">Price Over Time</TabsTrigger>
              <TabsTrigger value="snapshot" className="text-[10px] px-3 py-1">Current Snapshot</TabsTrigger>
            </TabsList>
              ...
              <div className="space-y-2">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-[9px] font-mono text-muted-foreground">
                    x^{currentParams.weightA.toFixed(2)} · y^{currentParams.weightB.toFixed(2)} = k
                  </span>
                </div>
                <div className="h-72">
                  <ResponsiveContainer width="100%" height="100%">
                    <LineChart data={curveData}>
                      ...
                      <XAxis dataKey="x" tick={{ fontSize: 9, fill: "hsl(var(--muted-foreground))" }} stroke="hsl(var(--muted-foreground))" />
            ...
            <TabsContent value="params" className="mt-4">
              <div className="space-y-4">
                ...
                <div className="h-56">
                  <ResponsiveContainer width="100%" height="100%">
                    <LineChart data={timeline}>
                      <CartesianGrid strokeDasharray="3 3" stroke="hsl(var(--border))" />
                      <XAxis dataKey="t" tick={{ fontSize: 9, fill: "hsl(var(--muted-foreground))" }} stroke="hsl(var(--muted-foreground))" label={{ value: "Time", position: "insideBottom", offset: -2, fontSize: 9 }} />
                      <YAxis tick={{ fontSize: 9, fill: "hsl(var(--muted-foreground))" }} stroke="hsl(var(--muted-foreground))" />
                      <Tooltip contentStyle={tooltipStyle} labelStyle={{ color: "hsl(var(--foreground))" }} />
                ...
                <div className="flex items-center gap-3 text-[9px] font-mono">
                  <div className="flex items-center gap-1"><div className="w-3 h-0.5 bg-chart-1 rounded" /> Weight A: {currentParams.weightA.toFixed(3)}</div>
                  <div className="flex items-center gap-1"><div className="w-3 h-0.5 bg-chart-2 rounded" /> Weight B: {currentParams.weightB.toFixed(3)}</div>
                  <div className="flex items-center gap-1"><div className="w-3 h-0.5 bg-chart-3 rounded" style={{ borderTop: "1px dashed" }} /> Fee: {currentParams.feeRate.toFixed(4)}</div>
                </div>
                ...
                <div className="h-40">
                  <ResponsiveContainer width="100%" height="100%">
                    <AreaChart data={timeline}>
                      <CartesianGrid strokeDasharray="3 3" stroke="hsl(var(--border))" />
                      <XAxis dataKey="t" tick={{ fontSize: 9, fill: "hsl(var(--muted-foreground))" }} stroke="hsl(var(--muted-foreground))" />
                      <YAxis tick={{ fontSize: 9, fill: "hsl(var(--muted-foreground))" }} stroke="hsl(var(--muted-foreground))" />
                      <Tooltip contentStyle={tooltipStyle} labelStyle={{ color: "hsl(var(--foreground))" }} />
            ...
            <TabsContent value="snapshot" className="mt-4">
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                  {[
                    { label: "Reserve X", value: currentSnap?.x?.toFixed(2) ?? "—", color: "chart-2" },
                    { label: "Reserve Y", value: currentSnap?.y?.toFixed(2) ?? "—", color: "chart-3" },
                    { label: "Price (Y/X)", value: currentSnap?.price?.toFixed(4) ?? "—", color: "chart-1" },
                    ...
                    <div key={i} className="p-3 rounded-lg bg-secondary border border-border">
                      ...
                      <p className={`text-sm font-mono font-semibold ${m.color ? `text-${m.color}` : "text-foreground"}`}>{m.value}</p>
                    </div>
                  ))}
                </div>
                ...
                <div className="p-3 rounded-lg bg-secondary border border-border">
                  ...
                  <p className="text-xs font-mono text-foreground">
                    x^{currentParams.weightA.toFixed(3)} · y^{currentParams.weightB.toFixed(3)} = k | fee={currentParams.feeRate.toFixed(4)} | A={currentParams.amplification.toFixed(1)}
                  </p>
                </div>
              </div>
            </TabsContent>

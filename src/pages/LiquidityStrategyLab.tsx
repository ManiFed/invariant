...
{ id: "montecarlo", label: "Monte Carlo", icon: Cpu, desc: "Stress-test with simulated price paths", step: 6 },
{ id: "arbitrage", label: "Arbitrage", icon: Search, desc: "Model toxic flow and MEV extraction", step: 7 },
{ id: "liquidity", label: "Liquidity", icon: BarChart3, desc: "Compare depth, efficiency, slippage", step: 8 },
{ id: "stability", label: "Stability", icon: Shield, desc: "Run diagnostic checks for edge cases", step: 9 },
{ id: "deploy", label: "Deploy", icon: Rocket, desc: "Export as Solidity, JSON, or docs", step: 10 }] as
const;
...
{ id: "uniswap-v2", name: "Uniswap V2", formula: "x * y = k", params: { wA: 0.5, wB: 0.5, k: 10000 } },
{
  ...
  formula: "An²Σxᵢ + D = ADn² + D^(n+1)/(n²Πxᵢ)",
  params: { wA: 0.5, wB: 0.5, k: 10000, amp: 100 }
},
{
  id: "balancer-weighted",
  name: "Balancer Weighted Pool",
  formula: "x^w₁ · y^w₂ = k",
  params: { wA: 0.8, wB: 0.2, k: 10000 }
},
{
  id: "uniswap-v3",
  name: "Uniswap V3 Concentrated",
  formula: "(√x − √pₐ)(√y − √p_b) = L²",
  params: { wA: 0.5, wB: 0.5, k: 10000 }
},
{ id: "solidly-ve33", name: "Solidly ve(3,3)", formula: "x³y + xy³ = k", params: { wA: 0.5, wB: 0.5, k: 10000 } },
{
  id: "power-perp",
  name: "Power Perpetual Curve",
  formula: "x^0.3 · y^0.7 = k",
  params: { wA: 0.3, wB: 0.7, k: 10000 }
}];
...
function loadStrategies(): StrategyConfig[] {
  try {
    const raw = sessionStorage.getItem(STRATEGY_SESSION_KEY);
...
const LiquidityStrategyLab = () => {
  ...
  const feeRate =
  savedFees && savedFees.length > 0 ? savedFees.reduce((a, b) => a + b, 0) / savedFees.length / 10000 : 0.003;
  ...
  <div className="surface-elevated rounded-xl p-4 mb-4">
      <div className="flex items-center gap-2 mb-2">
        ...
        <span className="text-[9px] font-mono px-1.5 py-0.5 rounded bg-chart-4/10 text-chart-4">
      ...
      <div
        key={s.id}
        className="inline-flex items-center gap-1.5 px-2 py-1 rounded-md bg-secondary border border-border text-[9px] font-mono">
        ...
        <aside
          ...
          <nav className="flex-1 py-2">
            {tabs.map((tab) => {
              ...
              const isLocked =
              tab.id !== "invariant" && !hasInvariant ||
              tab.id === "backtest" && !hasStrategies ||
          ...
          <motion.div
            ...
            className="mb-4 p-3 rounded-lg bg-warning/10 border border-warning/20 flex items-center gap-2">
              ...
              <p className="text-xs text-foreground">
                <strong>Step 1:</strong> Design your invariant below, then click{" "}
                <strong>"Set as Active Invariant"</strong> to unlock the other tools.
              </p>
            </motion.div>
          }

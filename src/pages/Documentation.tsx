        ...
        <ResponsiveContainer width="100%" height="100%">
          <LineChart data={data}>
            ...
            <XAxis
              ...
              label={{ value: "Trade Size (% of reserves)", fontSize: 9, position: "bottom" }}
            />
            <YAxis tick={{ fontSize: 9 }} stroke="hsl(var(--muted-foreground))" unit="%" />
            <Tooltip
              contentStyle={{
...
function InteractiveWidget({ type }: { type: string }) {
  switch (type) {
    case "il-calculator":
      return <ILCalculator />;
    case "slippage-explorer":
      return <SlippageExplorer />;
    case "fee-breakeven":
      return <FeeBreakevenCalc />;
    default:
      return null;
  }
}
      ...
      <div className="flex">
            ...
            <nav className="space-y-0.5">
              {documentationSections.map((section) => (
                <div key={section.id}>
                  ...
                  >
                    <ChevronRight
                      className={`w-3 h-3 transition-transform shrink-0 ${expandedSections.has(section.id) ? "rotate-90" : ""}`}
                    />
        ...
        <main className="flex-1 max-w-3xl mx-auto px-6 sm:px-8 py-10">
          ...
          <div className="space-y-14">
            {filteredSections.map((section) => (
              <div key={section.id} id={section.id}>
                <motion.h2
                  className="text-xl font-bold text-foreground mb-5 border-b border-border pb-2"
                  initial={{ opacity: 0 }}
          ...
          <footer className="mt-16 pt-8 border-t border-border text-center text-xs text-muted-foreground">
            Invariant Studio — Platform Documentation v3.0 —{" "}
            {documentationSections.reduce((n, s) => n + s.subsections.length, 0)} sections
          </footer>
        </main>
      </div>
    </div>
...
function renderContent(content: string) {
  return content.split("\n\n").map((paragraph, j) => {
    if (paragraph.startsWith("**") && paragraph.endsWith("**") && !paragraph.includes("\n")) {
      return (
        <h4 key={j} className="text-xs font-bold text-foreground mt-5 mb-1.5">
          {paragraph.replace(/\*\*/g, "")}
        </h4>
      );
    }
    if (paragraph.startsWith("    ")) {
      return (
        <pre
          key={j}
          className="bg-secondary border border-border rounded-lg px-4 py-3 my-3 text-[11px] font-mono text-foreground overflow-x-auto"
        >
          {paragraph.trim()}
        </pre>
      );
          ...
          <table className="w-full text-[11px] border border-border rounded-lg overflow-hidden">
            <thead>
              <tr className="bg-secondary">
                {headers?.map((h, hi) => (
                  <th key={hi} className="text-left py-1.5 px-2.5 text-muted-foreground font-medium">
                    {h}
                  </th>
                ))}
              </tr>
              ...
              {bodyRows.map((row, ri) => (
                <tr key={ri} className="border-t border-border">
                  {row
                    ...
                    .filter(Boolean)
                    .map((cell, ci) => (
                      <td key={ci} className="py-1.5 px-2.5 font-mono text-foreground">
                        {cell.trim()}
                      </td>
                    ))}
                </tr>
    ...
    if (paragraph.startsWith("- ") || paragraph.startsWith("1. ")) {
      ...
      return (
        <ul key={j} className="space-y-0.5 my-2">
          {items.map((item, k) => (
            <li key={k} className="text-xs text-muted-foreground leading-relaxed pl-1">
              {renderInline(item)}
            </li>
          ))}
        </ul>
    ...
    return (
      <p key={j} className="text-xs text-muted-foreground leading-relaxed mb-2">
        {parts.map((part, k) =>
          part.startsWith("**") && part.endsWith("**") ? (
            ...
            <span key={k}>{renderCode(part)}</span>
          ),
        )}
      </p>
    );
...
function renderCode(text: string) {
  ...
  return parts.map((part, k) =>
    part.startsWith("`") && part.endsWith("`") ? (
      <code key={k} className="px-1 py-0.5 bg-secondary rounded text-[10px] font-mono text-foreground">
        {part.replace(/`/g, "")}
      </code>
    ) : (
      part

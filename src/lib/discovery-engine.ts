...
export interface Candidate {
  ...
  stability: number;        // cross-path std-dev of lpValueVsHodl
...
export function priceImpact(bins: Float64Array, refLogPrice: number, tradeSize: number, direction: "buy" | "sell"): { output: number; slippage: number; newLogPrice: number } {
  // Find the bin containing the current price (clamped to valid bounds)
  const clampedRefLogPrice = Math.max(LOG_PRICE_MIN, Math.min(LOG_PRICE_MAX, refLogPrice));
  let remaining = Math.abs(tradeSize);
  let output = 0;
  let currentLogPrice = clampedRefLogPrice;
  const startBin = Math.min(
    NUM_BINS - 1,
    Math.max(0, Math.floor((currentLogPrice - LOG_PRICE_MIN) / BIN_WIDTH))
  );
  ...
  if (direction === "buy") {
    ...
    for (let i = startBin; i >= 0 && remaining > 0; i--) {
      ...
      remaining -= consumed;
      currentLogPrice = Math.max(LOG_PRICE_MIN, Math.min(LOG_PRICE_MAX, binCenter(i) + BIN_WIDTH * 0.5));
    }
  } else {
  ...
  const refPrice = Math.exp(clampedRefLogPrice);
  const idealOutput = direction === "buy"
    ? Math.abs(tradeSize) / refPrice
    : Math.abs(tradeSize) * refPrice;
...
function executeRandomTrade(state: SimState): void {
  ...
  const { slippage, newLogPrice } = priceImpact(state.bins, state.currentLogPrice, effectiveSize, direction);

  state.totalFees += fee;
  state.totalSlippage += slippage * effectiveSize;
  state.currentLogPrice = newLogPrice;
  state.tradeCount++;
}

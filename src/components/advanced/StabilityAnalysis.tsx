...
const StabilityAnalysis = () => {
  ...
  const checks = useMemo(() => {
    const base: { id: string; label: string; status: "pass" | "warn" | "fail"; detail: string }[] = [
      {
        id: "insolvency",
        label: "Insolvency Edge Cases",
        status: invariant === "cl" ? "warn" : "pass",
        detail: invariant === "cl"
          ? "Concentrated positions can reach zero liquidity if price exits range"
          : "Pool maintains solvency across all tested price ranges",
      },
      {
        id: "path_dep",
        label: "Path Dependence",
        status: feeRate > 0.5 ? "warn" : "pass",
        detail: feeRate > 0.5
          ? "High fee tiers introduce measurable path dependence in LP returns"
          : "Path dependence is negligible at current fee tier",
      },
      {
        id: "fee_distortion",
        label: "Fee Distortion",
        status: feeRate > 0.8 ? "fail" : feeRate > 0.3 ? "warn" : "pass",
        detail: feeRate > 0.8
          ? "Fee distortion exceeds safe thresholds — arbitrageurs may exploit mispricing"
          : "Fee distortion within acceptable bounds",
      },
      {
        id: "inventory",
        label: "Inventory Runaway",
        status: invariant === "wt" ? "pass" : volatility > 120 ? "fail" : volatility > 60 ? "warn" : "pass",
        detail: volatility > 120
          ? "High volatility creates inventory imbalance risk beyond rebalance capacity"
          : "Inventory drift is manageable under current volatility assumptions",
      },
      {
        id: "reflexivity",
        label: "Reflexivity Loops",
        status: invariant === "cl" && volatility > 100 ? "fail" : invariant === "cl" ? "warn" : "pass",
        detail: invariant === "cl" && volatility > 100
          ? "Concentrated positions + high vol create reflexive liquidation cascades"
          : "No significant reflexivity detected",
      },
    ];
    return base;
  ...
  const stressData = useMemo(() => {
    ...
    for (let i = 0; i <= 50; i++) {
      ...
      const cpStress = baseStress * 1;
      const selectedStress = invariant === "cp" ? cpStress
        : invariant === "ss" ? baseStress * 0.3
        : invariant === "wt" ? baseStress * 0.7
        : baseStress * 1.8;
      data.push({
        scenario,
        stress: parseFloat(selectedStress.toFixed(2)),
        threshold: 5,
      });
    }
    return data;
    ...
    <div className="space-y-6">
      ...
      <div className="surface-elevated rounded-xl p-5">
        <div className="flex items-center gap-2 mb-4">
          <Shield className="w-4 h-4 text-chart-purple" />
        ...
        <div className="grid grid-cols-3 gap-4">
            ...
            <div className="flex gap-1.5">
              {([["cp", "x·y=k"], ["ss", "Stable"], ["wt", "Weighted"], ["cl", "Conc."]] as [InvariantType, string][]).map(([id, label]) => (
                <button key={id} onClick={() => setInvariant(id)}
                  className={`flex-1 py-1.5 rounded-md text-[10px] font-medium transition-all ${invariant === id ? "bg-chart-purple/10 text-chart-purple border border-chart-purple/30" : "bg-secondary text-secondary-foreground border border-transparent"}`}>
                  {label}
                </button>
              ))}
            </div>
          ...
          <div>
            <div className="flex justify-between mb-1">
              ...
              <span className="text-[10px] font-mono text-foreground">{feeRate.toFixed(2)}</span>
            </div>
            <input type="range" min={0.01} max={1} step={0.01} value={feeRate} onChange={e => setFeeRate(Number(e.target.value))} className="w-full accent-chart-purple h-1" />
          </div>
          <div>
            <div className="flex justify-between mb-1">
              <label className="text-[10px] text-muted-foreground">Volatility (%)</label>
              <span className="text-[10px] font-mono text-foreground">{volatility}</span>
            </div>
            <input type="range" min={10} max={200} step={5} value={volatility} onChange={e => setVolatility(Number(e.target.value))} className="w-full accent-chart-purple h-1" />
          </div>
        </div>
      </div>
      ...
      <div className="space-y-2">
        {checks.map((check, i) => (
          <motion.div
            key={check.id}
            className="surface-elevated rounded-xl p-4 flex items-start gap-3"
        ...
        <div className="h-48">
          <ResponsiveContainer width="100%" height="100%">
            <AreaChart data={stressData}>
              ...
              <XAxis dataKey="scenario" tick={{ fontSize: 9, fill: "hsl(215, 15%, 50%)" }} />
              ...
              <Area type="monotone" dataKey="threshold" stroke="hsl(40, 90%, 55%)" strokeDasharray="5 5" fill="none" strokeWidth={1} />
            </AreaChart>
          </ResponsiveContainer>
        </div>
      </motion.div>

...
import { REGIMES, generatePricePath, NUM_BINS, TOTAL_LIQUIDITY, binCenter, FEE_RATE, ARB_THRESHOLD, DT, FAST_PATH_STEPS } from "@/lib/discovery-engine";
...
function simulateStep(
...
): { fees: number; lpValue: number; slippage: number } {
  let totalFees = 0;
  let totalSlippage = 0;
  ...
  // Simplified slippage estimate from local density
  const priceBin = Math.floor((externalLogPrice - (-2)) / (4 / NUM_BINS));
  const localDensity = priceBin >= 0 && priceBin < NUM_BINS ? bins[priceBin] : 1;
  totalSlippage = localDensity > 0 ? (tradeSize * 0.001) / localDensity : 0.01;
  ...
  for (let i = 0; i < NUM_BINS; i++) {
    ...
    if (center < externalLogPrice) reserveY += bins[i];
    else reserveX += bins[i];
  }
  const price = Math.exp(externalLogPrice);
  const lpValue = reserveX * price + reserveY + totalFees;

  return { fees: totalFees, lpValue, slippage: totalSlippage };
}
...
export default function ArenaView({ state, onSelectCandidate }: ArenaViewProps) {
  ...
  const runBattle = useCallback(() => {
    ...
    for (let t = 0; t <= FAST_PATH_STEPS; t++) {
      ...
      if (t > 0) {
        const stepA = simulateStep(a.bins, logPriceA, extPrice);
        const stepB = simulateStep(b.bins, logPriceB, extPrice);
        feesA += stepA.fees;
        feesB += stepB.fees;
        logPriceA = extPrice;
        logPriceB = extPrice;

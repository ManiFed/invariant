...
export const ASSET_COLORS = ["hsl(var(--chart-1))", "hsl(var(--chart-2))", "hsl(var(--chart-3))", "hsl(var(--chart-4))", "hsl(var(--chart-5))"];

export function defaultAssets(): Asset[] {
  return [
    { id: "1", symbol: "ETH", reserve: 1000, weight: 0.5, color: ASSET_COLORS[0] },
    { id: "2", symbol: "USDC", reserve: 1000, weight: 0.3, color: ASSET_COLORS[1] },
    { id: "3", symbol: "BTC", reserve: 500, weight: 0.2, color: ASSET_COLORS[2] },
  ];
}
...
function computeSurface(assets: Asset[], axisI: number, axisJ: number, gridSize: number = 40): { x: number; y: number }[] {
...
/* ─── Asset Table ─── */
export function AssetTable({ assets, onChange, onAdd, onRemove }: {
  assets: Asset[];
  ...
  return (
    <div className="space-y-2">
      <div className="flex items-center justify-between">
        ...
        <button onClick={onAdd} disabled={assets.length >= 5}
            ...
            {assets.map((a, i) => (
              <tr key={a.id} className="border-b border-border/50">
                <td className="py-1.5 px-2">
                  <div className="w-4 h-4 rounded-full flex items-center justify-center text-[8px] font-bold" style={{ backgroundColor: a.color + "20", color: a.color }}>{i + 1}</div>
                </td>
                <td className="py-1.5 px-2">
                  <input value={a.symbol} onChange={e => updateAsset(a.id, "symbol", e.target.value)}
                    className="w-16 px-1.5 py-1 bg-secondary rounded border border-border font-mono text-foreground" />
                </td>
                <td className="py-1.5 px-2">
                  <input type="number" value={a.reserve} onChange={e => updateAsset(a.id, "reserve", Math.max(1, Number(e.target.value)))}
                    className="w-20 px-1.5 py-1 bg-secondary rounded border border-border font-mono text-foreground" />
                </td>
                <td className="py-1.5 px-2">
                  <input type="number" step={0.05} min={0.01} max={0.99} value={a.weight}
      ...
      <div className="flex items-center gap-2 text-[9px]">
        ...
        <span className={`font-mono font-bold ${Math.abs(totalWeight - 1) < 0.01 ? "text-success" : "text-warning"}`}>{totalWeight.toFixed(2)}</span>
      ...
      <div className="p-2.5 rounded-lg bg-secondary border border-border">
        ...
        <p className="text-xs font-mono text-foreground">{assets.map(a => `${a.symbol}^${a.weight}`).join(" · ")} = k</p>
      </div>
    </div>
  );
}
...
export function Surface3D({ assets, axisI, axisJ }: { assets: Asset[]; axisI: number; axisJ: number }) {
  const chartData = useMemo(() => computeSurface(assets, axisI, axisJ, 40).map(p => ({ x: p.x, y: p.y })), [assets, axisI, axisJ]);
  return (
    <div className="h-64">
      <div className="flex items-center gap-2 mb-2">
        <span className="text-[9px] font-mono text-muted-foreground">{assets[axisI].symbol} vs {assets[axisJ].symbol} invariant surface</span>
      </div>
      <ResponsiveContainer width="100%" height="100%">
        <LineChart data={chartData}>
          ...
          <Tooltip contentStyle={{ backgroundColor: "hsl(var(--background))", border: "1px solid hsl(var(--border))", borderRadius: "8px", fontSize: "10px", color: "hsl(var(--foreground))" }} labelStyle={{ color: "hsl(var(--foreground))" }} />
...
export function PriceHeatmap({ assets }: { assets: Asset[] }) {
  const matrix = useMemo(() => assets.map((ai) => assets.map((aj, j) => ai.id === aj.id ? 1 : spotPrice(ai.reserve, ai.weight, aj.reserve, aj.weight))), [assets]);
  const maxVal = Math.max(...matrix.flat().filter(v => v !== 1));
  return (
    <div className="space-y-2">
      <h4 className="text-[10px] font-bold text-foreground flex items-center gap-1.5"><Grid3X3 className="w-3 h-3" /> Price Matrix (∂xⱼ/∂xᵢ)</h4>
      <div className="overflow-x-auto">
        <table className="text-[9px] font-mono">
          <thead><tr><th className="px-2 py-1 text-muted-foreground"></th>{assets.map(a => <th key={a.id} className="px-2 py-1 text-muted-foreground text-center">{a.symbol}</th>)}</tr></thead>
          <tbody>
            {assets.map((ai, i) => (
              <tr key={ai.id}>
                ...
                {matrix[i].map((val, j) => {
                  ...
                  return <td key={j} className="px-2 py-1 text-center rounded" style={{ backgroundColor: i === j ? "hsl(var(--secondary))" : `hsla(var(--chart-1), ${0.1 + intensity * 0.5})`, color: "hsl(var(--foreground))" }}>{val.toFixed(4)}</td>;
                })}
              </tr>
            ))}
          </tbody>
...
export function LiquidityRadar({ assets }: { assets: Asset[] }) {
  const data = useMemo(() => assets.map(a => ({ subject: a.symbol, reserve: a.reserve, weight: a.weight * 1000, liquidity: a.reserve * a.weight })), [assets]);
  return (
    <div className="space-y-2">
      <h4 className="text-[10px] font-bold text-foreground flex items-center gap-1.5"><Radar className="w-3 h-3" /> Liquidity Distribution</h4>
      <div className="h-48">
        <ResponsiveContainer width="100%" height="100%">
...
export function DerivedMetrics({ assets }: { assets: Asset[] }) {
  const pairs = useMemo(() => {
    ...
    for (let i = 0; i < assets.length; i++) {
      for (let j = 0; j < assets.length; j++) {
        ...
        const out = swapOutCalc(assets[i].reserve, assets[i].weight, assets[j].reserve, assets[j].weight, assets[i].reserve * 0.01);
  ...
  return (
    <div className="space-y-2">
      <h4 className="text-[10px] font-bold text-foreground flex items-center gap-1.5"><Settings2 className="w-3 h-3" /> Swap Rates (1% of reserve)</h4>
      <div className="overflow-x-auto">
        <table className="w-full text-[9px] font-mono">
          <thead><tr className="border-b border-border"><th className="text-left py-1 px-1.5 text-muted-foreground">Pair</th><th className="text-right py-1 px-1.5 text-muted-foreground">Spot Rate</th><th className="text-right py-1 px-1.5 text-muted-foreground">Output (1%)</th></tr></thead>
          <tbody>
            {pairs.map((p, i) => (
...
export function MultiAssetOptimizer({ assets, onApply }: { assets: Asset[]; onApply: (weights: number[]) => void }) {
  ...
  const optimize = () => {
    ...
    switch (objective) {
      case "equal": newWeights = assets.map(() => 1 / assets.length); break;
      case "minSlippage": { const total = assets.reduce((s, a) => s + a.reserve, 0); newWeights = assets.map(a => a.reserve / total); break; }
      case "maxEfficiency": { const total = assets.reduce((s, a) => s + Math.sqrt(a.reserve), 0); newWeights = assets.map(a => Math.sqrt(a.reserve) / total); break; }
    }
    onApply(newWeights);
  };
  return (
    <div className="space-y-3 p-3 rounded-lg bg-secondary border border-border">
      ...
      <div className="space-y-1">
        {([
          ...
          <button key={opt.id} onClick={() => setObjective(opt.id)}
      ...
      <button onClick={optimize} className="w-full px-3 py-2 text-[10px] font-medium rounded-lg bg-primary text-primary-foreground hover:opacity-90 transition-opacity">Apply Optimization</button>
    </div>
  );
}

/* ─── Pairwise Slice Selector ─── */
export function PairwiseSliceSelector({ assets, axisI, axisJ, onChangeI, onChangeJ, fixedValues, onFixedChange }: {
  assets: Asset[]; axisI: number; axisJ: number; onChangeI: (i: number) => void; onChangeJ: (j: number) => void; fixedValues: number[]; onFixedChange: (idx: number, val: number) => void;
}) {
  return (
    <div className="space-y-2 p-3 rounded-lg bg-secondary border border-border">
      <h4 className="text-[10px] font-bold text-foreground flex items-center gap-1.5"><Crosshair className="w-3 h-3" /> Pairwise Slice Selector</h4>
      <div className="grid grid-cols-2 gap-2">
        <div>
          <label className="text-[9px] text-muted-foreground">X Axis</label>
          <select value={axisI} onChange={e => onChangeI(Number(e.target.value))} className="w-full mt-0.5 px-2 py-1 text-[10px] bg-background border border-border rounded text-foreground">
            {assets.map((a, i) => <option key={i} value={i} disabled={i === axisJ}>{a.symbol}</option>)}
          </select>
        </div>
        <div>
          <label className="text-[9px] text-muted-foreground">Y Axis</label>
          <select value={axisJ} onChange={e => onChangeJ(Number(e.target.value))} className="w-full mt-0.5 px-2 py-1 text-[10px] bg-background border border-border rounded text-foreground">
            {assets.map((a, i) => <option key={i} value={i} disabled={i === axisI}>{a.symbol}</option>)}
          </select>
        </div>
      </div>
      {assets.length > 2 && (
          ...
          {assets.map((a, i) => {
            ...
            return (
              <div key={i} className="flex items-center gap-2">
                ...
                <Slider min={1} max={a.reserve * 3} step={1} value={[fixedValues[i] ?? a.reserve]} onValueChange={([v]) => onFixedChange(i, v)} className="flex-1" />
...
export function MultiAssetPanel({ assets, onChange, onAdd, onRemove, onApplyWeights }: {
  assets: Asset[]; onChange: (a: Asset[]) => void; onAdd: () => void; onRemove: (id: string) => void; onApplyWeights: (w: number[]) => void;
}) {
          ...
          <div className="flex gap-1.5 flex-wrap">
            {(["surface", "heatmap", "radar", "metrics"] as const).map(m => (
              <button key={m} onClick={() => setVizMode(m)}
                ...
                {m === "surface" ? "Surface Plot" : m === "heatmap" ? "Price Heatmap" : m === "radar" ? "Liquidity Radar" : "Swap Metrics"}
              </button>
            ))}
          </div>
          {vizMode === "surface" && (
            <div className="space-y-4">
              {assets.length > 2 && (
                <PairwiseSliceSelector assets={assets} axisI={axisI} axisJ={axisJ} onChangeI={setAxisI} onChangeJ={setAxisJ} fixedValues={fixedValues} onFixedChange={handleFixedChange} />
              )}
